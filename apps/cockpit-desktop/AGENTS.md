...
- `npm run tauri:dev` prepares the simulator sidecar and launches the desktop app. This is the canonical development launcher; it triggers Tauri's `beforeDevCommand`, which starts the Vite dev server on `127.0.0.1:15342`. From the repository root you can run `./run-desktop.sh` to do the same thing with extra diagnostics.
