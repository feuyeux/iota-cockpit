...
export function SimulationSourcePanel({ model, dispatch, onEvaluationCompleted }: Props) {
  ...
  async function autoRunScenario() {
    ...
    try {
      ...
      // observed.
      const maxTicks = loaded.scenario.maxTicks + 1;
      ...
      if (!autoRunCancelled.current && !terminalStatus) {
        throw new Error(`Scenario did not reach a terminal state within ${maxTicks} ticks`);
      }
