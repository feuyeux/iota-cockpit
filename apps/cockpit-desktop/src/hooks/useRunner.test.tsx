...
function Harness() {
  ...
  const { runCommand } = useRunner(model, dispatch);
...
describe("useRunner", () => {
  it("keeps the Runner timeout error and failed terminal state after a command rejection", async () => {
    vi.spyOn(runnerClient, "snapshot").mockResolvedValue({
      events: [
        {
