...
use cockpit_simulation_core::error::SimulationError;
...
fn extract_json_objects(text: &str) -> Vec<&str> {
    text.char_indices()
        ...
        .filter_map(|start| extract_balanced_object(&text[start..]).map(|end| &text[start..start + end]))
        .collect()
}
...
/// need to fail a [`cockpit_simulation_core::simulation::Simulation`] run
