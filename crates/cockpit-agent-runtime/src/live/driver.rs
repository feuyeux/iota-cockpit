//! [`HumanAgentDriver`]: the stateful per-tick orchestration that drives one
//! mandatory backend/tool loop per human.
//!
...
use crate::{
    LocalMcpServer, OpenWorldControlRequest, TOOL_REQUEST_ACTION, ToolRequest,
    open_world::OpenWorldRuntime, redact_json,
};

use super::{
    MAX_ACTIONS_PER_DECISION, MAX_TOOL_CALLS_PER_TURN, MAX_TOOL_COST_PER_TURN,
    REDACTED_DECISION_TEXT,
    decision::{parse_decision, parse_turn_output, redact_decision_prose, sanitize_decision},
    tool_call_cost,
    types::{
        ...
        for human_id in &human_ids {
            ...
            let mut decision = loop {
                ...
                match parse_turn_output(&text).map_err(|reason| HumanTurnError::InvalidOutput {
                    ...
                    HumanTurnOutput::Final(decision) => break decision,
