...
//! and authorized tool schemas but no eager complete observation. It may return
//! up to eight `toolCall` outputs; each result is fed into the next round. A
//! `final` output ends the person's tick and may include:
...
//! `cockpit_agent_runtime::live::*` call sites are unaffected by this split.
