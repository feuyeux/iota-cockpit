...
pub const OPEN_WORLD_RUNTIME_VERSION: u32 = 2;
...
impl AgentSessionState {
    fn new(agent_id: &str, initial_goal: &str, tick: u64) -> Self {
        ...
        .collect();
        let skills = [(
            "cockpit-simulation".to_string(),
            SkillState {
                skill_id: "cockpit-simulation".to_string(),
                version: "4".to_string(),
                lifecycle: ResourceLifecycle::Active,
        ...
        Self {
            ...
            plan: vec![PlanStep {
                ...
                required_tool: Some("simulation.get_observation".to_string()),
...
pub struct OpenWorldCheckpoint {
    ...
    pub world: cockpit_simulation_core::WorldSnapshot,
    pub runtime: OpenWorldRuntime,
}

impl OpenWorldCheckpoint {
    pub fn capture(
        world: &cockpit_simulation_core::WorldSnapshot,
...
mod tests {
    ...
    fn scheduler_is_priority_bounded_and_runtime_restores() {
        ...
        assert_eq!(
            runtime.sessions["b"].skills["cockpit-simulation"].lifecycle,
            ResourceLifecycle::Active
        );
        assert_eq!(runtime.sessions["b"].tools.len(), 8);
