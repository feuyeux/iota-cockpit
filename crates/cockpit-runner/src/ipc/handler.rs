//! `RunnerHandler` core: state, constructors, IPC request dispatch/routing,
...
//! RunnerHandler` block; no method changed behavior as part of the split.
...
impl RunnerHandler {
    ...
    /// evaluator. The Runner does not load rubrics or perform scoring.
    ...
    pub async fn dispatch_async(&mut self, request: RunnerRequest) -> RunnerResponse {
        if !matches!(
            ...
            RunnerCommand::CreateLiveSimulationRun { .. }
                | RunnerCommand::ResumeLiveSimulation { .. }
    ...
    pub fn dispatch(&mut self, request: RunnerRequest) -> RunnerResponse {
        ...
        let result = match request.command {
            ...
            RunnerCommand::SpawnEntity { entity } => self.spawn_entity(entity),
            RunnerCommand::RemoveEntity { entity_id } => self.remove_entity(&entity_id),
            RunnerCommand::AddAgentGoal {
                agent_id,
                description,
                priority,
            } => self.add_agent_goal(&agent_id, description, priority),
            RunnerCommand::SetAgentGoalStatus {
                agent_id,
                goal_id,
                status,
            } => self.set_agent_goal_status(&agent_id, &goal_id, status),
            RunnerCommand::WaitAgentUntil {
                agent_id,
                wake_tick,
            } => self.wait_agent_until(&agent_id, wake_tick),
            RunnerCommand::GetOpenWorldRuntime => {
                serde_json::to_value(self.live_driver.open_world())
                    .map_err(|error| Box::new(Self::serialization_error(error.to_string())))
            }
            RunnerCommand::CheckpointOpenWorld => self.checkpoint_open_world(),
    ...
    pub(super) fn emit(&mut self, event: RunnerEvent) {
        self.next_cursor += 1;
    ...
    pub(super) fn tick_committed_event(snapshot: &WorldSnapshot) -> RunnerEvent {
        RunnerEvent::SimulationTickCommitted {
            cursor: 0,
    ...
    pub(super) fn tool_error(error: cockpit_agent_runtime::ToolError) -> IpcError {
        IpcError {
            code: error.code,
