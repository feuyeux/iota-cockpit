//! External runner-process recovery across a real process restart.
//!
//! Spawns the actual `cockpit-runner serve --recording-db` binary over
...
fn spawn_runner(address: &SocketAddr, database: &str) -> Child {
    let child = Command::new(env!("CARGO_BIN_EXE_cockpit-runner"))
        .args([
            "serve",
            "--bind",
            &address.to_string(),
...
#[test]
fn external_runner_process_recovers_after_restart() {
    ...
    first.kill().expect("first runner killed");
    first.wait().expect("first runner reaped");
    sleep(Duration::from_millis(100));

    // Second process against the same database recovers via resume.
    let mut second = spawn_runner(&address, &database);
    let resumed = request(
        &address,
        json!({
    ...
    second.kill().expect("second runner killed");
    second.wait().expect("second runner reaped");
