...
impl CapabilityCatalog {
    ...
    pub fn from_yaml(bytes: &str) -> Result<Self, String> {
        let document: CapabilityCatalogDocument =
            serde_yaml::from_str(bytes).map_err(|error| format!("invalid capability catalog YAML: {error}"))?;
...
fn write_alarm_field(snapshot: &mut WorldSnapshot, path: &str, value: &JsonValue) -> Result<(), String> {
    match path {
        "active" => {
...
fn write_cabin_field(snapshot: &mut WorldSnapshot, path: &str, value: &JsonValue) -> Result<(), String> {
    let as_bool = || {
        value
            .as_bool()
