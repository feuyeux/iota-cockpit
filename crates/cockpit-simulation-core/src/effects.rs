...
impl EffectPlan {
    pub fn apply(&self, snapshot: &mut WorldSnapshot) -> Result<(), ErrorCode> {
        for operation in &self.operations {
            write_field(snapshot, &operation.entity_id, &operation.path, &operation.value)
...
fn build_effect_plan(
    ...
    for event in &capability.events {
        let target = match &event.target {
            Some(target) => {
                Some(resolve_entity_id(target, snapshot, &intent.target_id, &intent.actor_id)
                    .ok_or(ErrorCode::UnknownTarget)?)
            }
            None => None,
        };
...
fn action_already_applied(snapshot: &WorldSnapshot, capability: &CapabilityDefinition) -> bool {
    ...
    match capability.id.as_str() {
        ...
        "driver.activateFatigueIntervention" => systems.driver_assistance.fatigue_intervention_active,
