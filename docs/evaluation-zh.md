...
- `cockpit-runner` 只执行模拟并产出 Recording，不读取私有 rubric，也不决定发布门禁；
...
从 Runner 导出的 JSON Recording 评测：
...
- `scenario`：evaluator 调用 `--runner-command` 生成 Recording；
...
单个 case 的 rubric、Recording、Runner 或 evaluator 错误不会提前终止整个 case 循环。该 case 会写入：
...
2. Tauri 从 Runner 获取 Recording，并校验 Recording 的 scenario 与请求的私有 rubric 一致；
...
Embedded 模式直接读取 Runner 内存中的 Recording。Process 模式读取 SQLite 后，还会通过已鉴权 IPC 调用 `GetSimulationSnapshot`，并比较：

- Recording `runId` 与当前 Runner snapshot `runId`；
- durable tick 与 snapshot tick。durable tick 等于最后一条 `StepRecord.tick + 1`，空 Recording 为 `0`。

任一项不一致都拒绝评测，避免持久化失败或落后一 tick 时对陈旧 Recording 打分。用户应先处理 Runner 持久化错误，再重新评测，而不是绕过该校验。
...
仓库根目录是 monorepo，Rust workspace 位于 `cockpit-simulator/`。CI 的 Rust job 在该目录运行，并在无 Cargo cache 的干净环境执行 `cargo fetch --locked`，验证锁定的 crates.io 包 `iota-sympantos-core` 可解析，不依赖旧 Git revision。

发布门禁构建 Runner/evaluator 后执行默认 10 场景 suite，并始终上传：

- `cockpit-simulator/target/evaluation-report.json`；
- `cockpit-simulator/target/evaluation-junit.xml`。

Tauri `externalBin` 同时声明：

- `binaries/cockpit-runner`；
...
- Runner restart 合约使用普通 persistent run 写入 open-world goal/checkpoint，销毁并重建 `RunnerHandler` 后验证 SQLite checkpoint 与 `ResumeSimulation`；该测试不启动 live ACP，因此不会因 workspace feature unification 启用真实 Hermes 而不稳定；
...
| 现象 | 检查 |
| :--- | :--- |
| 单个 suite case 为 `inconclusive` | 查看 `infrastructureError`、JUnit `<failure>` 和 Runner stderr 摘要 |
| suite 直接返回错误且无报告 | 检查 manifest/schema、重复 ID、baseline JSON 和 suite 级路径 |
| Desktop 提示 Recording 不处于当前 durable snapshot | 检查 Runner 持久化事件，确认 SQLite run ID/tick 已追上当前 snapshot |
