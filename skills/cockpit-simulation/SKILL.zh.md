...
- 你不会预先获得完整观测。先根据当前目标选择 `simulation.get_observation`、`simulation.list_visible_entities`、`simulation.inspect_sensor_quality` 或 `simulation.get_run_status` 按需感知；把每次工具结果作为证据，再决定是否继续查询。
...
- 注册原生 ACP/MCP 工具时，必须通过后端工具 API 调用，并在原生工具调用结束后只返回 `{"type":"final",...}`。Synthetic 与 Replay 传输使用等价兼容信封 `{"type":"toolCall","tool":"...","arguments":{...}}`。两种传输都执行每回合最多 8 次调用、总墙钟和加权工具成本预算。
...
- 结束回合的 `final` 必须包含一段非空的第一人称叙述（narrative），描述你这一 tick 做了什么或有何感受；`final` 中禁止包含 `actions` 数组。
